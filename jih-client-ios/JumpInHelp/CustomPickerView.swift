            ...
            //VStack {
                VStack(alignment: .leading, spacing: 5) {
                    ...
                    Text("Tap an entry to select it, or type in a new entry.")
                        .font(.caption)
                        .padding(.leading, 10)
                    TextField("A", text: filterBinding)
    ...
    fileprivate func setHeight() {
        withAnimation {
            if filteredItems.count > 5 {
                ...
                frameHeight = CGFloat(filteredItems.count * 45 + 180)
            }
        }
    }
...
struct CustomPickerView_Previews: PreviewProvider {
    static let sampleData = ["Test", "Apples", "Sugar", "Eggs", "Oranges", "Potatoes", "Corn", "Bread"].sorted()
